...
final class V2NamespaceConformanceCommandTest extends TestCase
{
    public function testCommandEmitsPhpNamespaceConformanceShard(): void
    {
        ...
        $http->fake(function (Request $request) use ($http, &$requests) {
            ...
            if ($method === 'GET' && $path === '/api/workflows/php-ns-test-tenant-a-workflow') {
                return $http->response([
            ...
            if ($method === 'POST' && $path === '/api/worker/workflow-tasks/poll') {
                ...
                if ($isTenantA && $pollNumber === 2) {
                    return $http->response([
                        'task' => [
                            'task_id' => 'task-a',
                            'workflow_id' => 'php-ns-test-tenant-a-workflow',
                            'workflow_task_attempt' => 1,
                            'lease_owner' => 'php-ns-test-tenant-a-worker',
                            'task_queue' => 'iso',
                        ],
                    ], 200);
                }
                ...
                if ($isTenantB && $pollNumber === 4) {
                    return $http->response([
                        'task' => [
                            'task_id' => 'task-b',
                            'workflow_id' => 'php-ns-test-tenant-b-workflow',
                            'workflow_task_attempt' => 1,
                            'lease_owner' => 'php-ns-test-tenant-b-worker',
                            'task_queue' => 'iso',
                        ],
                    ], 200);
                }
