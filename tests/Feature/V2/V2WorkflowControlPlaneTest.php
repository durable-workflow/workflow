...
final class V2WorkflowControlPlaneTest extends TestCase
{
    ...
    public function testPollModeStartProjectionAvoidsAncillaryRunRelationScans(): void
    {
        config()->set('workflows.v2.task_dispatch_mode', 'poll');

        $result = $this->controlPlane->start('remote-workflow-type', 'ctrl-plane-fresh-summary-fast-path', [
            'connection' => 'redis',
            'queue' => 'default',
        ]);
        ...
        foreach ($forbiddenTables as $table) {
            $this->assertFalse(
                $selects->contains(static fn (string $sql): bool => str_contains($sql, $table)),
                sprintf(
                    "Fresh start projection should not scan [%s].\nQueries:\n%s",
                    $table,
                    $selects->implode("\n"),
                ),
            );
