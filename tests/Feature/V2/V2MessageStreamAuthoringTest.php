...
final class V2MessageStreamAuthoringTest extends TestCase
{
    public function testWorkflowCanPeekReceiveAndReplyThroughFirstClassMessageStreams(): void
    {
        ...
        $this->assertSame(
            ['reply:payload:hello', 'reply:payload:hello'],
            $replyMessages->pluck('payload_reference')
                ->all(),
        );
        ...
        $this->assertSame('chat', $cursorAdvanced->payload['stream_key'] ?? null);
        $this->assertSame(0, $cursorAdvanced->payload['previous_position'] ?? null);
        $this->assertSame((int) $inbound->sequence, $cursorAdvanced->payload['new_position'] ?? null);
    }
