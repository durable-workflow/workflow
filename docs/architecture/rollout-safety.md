...
| `matching_role` | `queue_wake_enabled`, `shape`, `task_dispatch_mode` | matching-role deployment shape on the process serving the snapshot: `queue_wake_enabled` reports `workflows.v2.matching_role.queue_wake_enabled` exactly as the `WorkflowServiceProvider` `Looping` listener consumes it, `shape` is `in_worker` when this process still runs the in-worker broad-poll wake and `dedicated` when the process has opted out so the broad sweep runs under `php artisan workflow:v2:repair-pass` instead, and `task_dispatch_mode` reports the configured dispatch mode (`queue` or `poll`). The snapshot is process-local; in mixed-shape fleets, operators read one snapshot per node to confirm the opt-out was applied to the right nodes without inspecting config files |
