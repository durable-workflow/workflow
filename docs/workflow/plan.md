...
Durable Workflow supports three separate deployment choices:

- **Embedded Laravel** installs this package inside the application. The
  application owns orchestration, persistence, workers, and recovery.
- **Self-hosted Server** runs the standalone Server independently. SDK
  clients and remote workers connect to the Server endpoint; the Server
  operator owns its persistence, placement, and recovery.
- **Managed Cloud** gives SDK clients and workers a stable namespace
  endpoint. Cloud owns the orchestration service, persistence, regional
  placement, and recovery behind that endpoint.

These choices share workflow and worker-protocol semantics, but they are
not layers that customers assemble into one topology. In particular, a
self-hosted Server is never registered with Cloud or selected as the
backing service for a Cloud namespace. Cloud does not expose internal
placement identifiers or service addresses through the workflow package,
and the workflow package does not define Cloud administration,
connectivity, placement, or failover contracts.
...
| Worker compatibility and routing | Queue name and deployment convention | Compatibility marker on runs/tasks, `worker_compatibility_heartbeats`, routing snapshots | `WorkflowStarted` captures the definition fingerprint; run and task rows carry compatibility routing state | Claim rejection exposes compatibility mismatch as worker-plane outcome | `WorkerCompatibility`, `WorkerCompatibilityFleet`, `OperatorQueueVisibility`, Waterline worker/fleet panels | New in v2. Active v1 runs finish on v1; v2 runs stay on compatible workers through retries, children, and continue-as-new. |
...
- Custom serializer classes from v1 are not a v2 runtime contract.
  `php artisan workflow:v2:doctor` requires `workflows.serializer=avro`
  for the v2 runtime. The `workflow-serializer-y` and
  `workflow-serializer-base64` readers remain confined to the internal
  v1 drain/import boundary and are never selectable for new runs.
...
- `php artisan workflow:v2:doctor` reports the active backend matrix,
  flags unsupported settings (including any `workflows.serializer`
  value other than `avro`), and lists the readiness checks the
...
- Managed Cloud security, identity, and API lifecycle guarantees belong
  to the Cloud product contract. The workflow package owns only the
  namespace-scoped command, history, and worker-protocol facts that a
  managed namespace endpoint consumes.
...
- Service mode is the standalone-server distribution. The server owns
  ...
  `GET /api/cluster/info`. The engine role boundaries within an embedded
  or standalone deployment are described in
  [`docs/architecture/control-plane-split.md`](../architecture/control-plane-split.md).
- SDK clients and remote workers connect to the Server endpoint selected
  by the self-hosted operator. Service mode has no Cloud registration,
  Cloud placement, or Cloud failover dependency.
...
- [`docs/deployment/ha-failover.md`](../deployment/ha-failover.md),
  [`docs/deployment/multi-region.md`](../deployment/multi-region.md),
  and [`docs/deployment/multi-node-requirements.md`](../deployment/multi-node-requirements.md)
  define self-hosted HA, DR, region, replication, private-networking,
  backup, restore, and multi-node behavior.
