...
| `UpdateCompleted` | `workflow_command_id`, `update_id`, `workflow_instance_id`, `workflow_run_id`, `update_name`, `sequence`, `result`, `payload_codec`, `failure_id`, `failure_category`, `non_retryable`, `exception_type`, `exception_class`, `message`, `code`, `exception` | `RunUpdateView`, worker history payload consumers |
...
| `SideEffectRecorded` | `sequence`, `result`, `payload_codec` | `WorkflowStepHistory`, `WorkflowExecutor`, `QueryStateReplayer` |
