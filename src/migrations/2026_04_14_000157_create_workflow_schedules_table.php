    ...
    public function up(): void
    {
        Schema::create('workflow_schedules', static function (Blueprint $table): void {
            ...
            $table->unsignedBigInteger('remaining_actions')
                ->nullable();
            $table->unsignedBigInteger('fires_count')
                ->default(0);
            $table->unsignedBigInteger('failures_count')
                ->default(0);

            $table->json('recent_actions')
                ->nullable();
            $table->json('buffered_actions')
                ->nullable();

            $table->timestamp('last_fired_at', 6)
                ->nullable();
            $table->timestamp('next_fire_at', 6)
                ->nullable();

            $table->string('latest_workflow_instance_id', 191)
                ->nullable()
                ->index();
            ...
            $table->string('last_skip_reason', 64)
                ->nullable();
            $table->timestamp('last_skipped_at', 6)
                ->nullable();
            $table->unsignedBigInteger('skipped_trigger_count')
                ->default(0);
