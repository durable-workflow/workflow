    ...
    public function up(): void
    {
        Schema::create('workflow_child_calls', static function (Blueprint $table): void {
            ...
            $table->unsignedInteger('sequence')
                ->index(); // History event sequence that scheduled the child

            // Child identity (before resolution)
            $table->string('child_workflow_type', 191);
            $table->string('child_workflow_class', 255);
            $table->string('requested_child_id', 191)
                ->nullable(); // User-specified child instance ID

            // Resolved references (after child starts)
            $table->string('resolved_child_instance_id', 191)
                ->nullable()
                ->index();
            $table->string('resolved_child_run_id', 26)
                ->nullable()
                ->index();

            // Snapped parent-close policy
            $table->string('parent_close_policy', 32)
                ->default('abandon'); // abandon, request_cancel, terminate
            ...
            $table->string('queue', 191)
                ->nullable();
            $table->string('compatibility', 191)
                ->nullable();

            // Snapped policy options (for future expansion)
            $table->json('retry_policy')
                ->nullable();
            $table->json('timeout_policy')
                ->nullable();
            $table->boolean('cancellation_propagation')
                ->default(false);
            ...
            // Outcome metadata
            $table->string('result_payload_reference', 191)
                ->nullable();
            $table->string('failure_reference', 191)
                ->nullable();
            $table->string('closed_reason', 64)
                ->nullable(); // completed, failed, cancelled, terminated, abandoned
            ...
            $table->json('arguments')
                ->nullable(); // Serialized child workflow arguments
            $table->json('metadata')
                ->nullable(); // Extensibility
